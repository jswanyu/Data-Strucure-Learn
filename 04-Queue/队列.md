...
能够发现`ArrayQueue`和`LinkedList`即实现了`List`接口，又实现了`Queue`接口，所以在使用的时候，如果我们把它当作List，就获取List的引用，如果我们把它当作Queue，就获取Queue的引用
...
**所以无论是栈还是队列，JDK都是建议使用ArrayDeque而不是LinkedList实现，ArrayDeque比较复杂的一点就是需要指定初始大小，当然你不指定也行。但是它的效率确实是比LinkedList高的，虽然现在刷题基本上都是用LinkedList**
